...
```bash
wwe-superstars$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
wwe-superstars$ sam local start-api
wwe-superstars$ curl http://localhost:3000/
```
...
```bash
wwe-superstars$ sam logs -n HelloWorldFunction --stack-name wwe-superstars --tail
```
...
```bash
aws cloudformation delete-stack --stack-name wwe-superstars
```
